...
function addProductPrice(copy = false) {
    ...
    data = {
        ...
        cart_min: '',
        stock: '',
        productCode: '' // اضافه کردن فیلد جدید
    };
    ...
    if (copy && lastPrice.length) {
        ...
        data.productCode = lastPrice.find('.productCode').val(); // مقداردهی هنگام کپی
    ...
    price
        .find('input[name="productCode"]') // اضافه کردن فیلد جدید
        .attr('name', 'prices[' + count + '][productCode]')
